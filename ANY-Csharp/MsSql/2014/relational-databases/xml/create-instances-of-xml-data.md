...
```  
 ...
 You can also use the [TYPE directive](type-directive-in-for-xml-queries.md) in the FOR XML clause that directly returns a FOR XML query result as `xml` type:  
...
|Topic|Description|  
|-----------|-----------------|  
|[Retrieve and Query XML Data](retrieve-and-query-xml-data.md)|Describes the parts of XML instances that are not preserved when they are stored in databases.|  
